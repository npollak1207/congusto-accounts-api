...
async def bootstrap(current_user: dict = Depends(get_current_user)):
    ...
    if current_user.get("organization_id"):
        ...
        if org_result.data:
            org = org_result.data
    org_data = {"id": org["id"], "name": org["name"], "slug": org["slug"], "organizationName": org["name"], "organizationSlug": org["slug"], "chatMode": org.get("chat_mode", "standard"), "supportedRoles": ["property_manager", "contractor", "employee"], "accessibleProperties": 0} if org else None
    return {
        ...
        "stats": {"open_jobs": 0, "waiting_approval_jobs": 0, "accessibleProperties": 0, "unreadNotifications": 0, "totalJobs": 0, "openJobs": 0, "assignedJobs": 0, "inProgressJobs": 0, "waitingApprovalJobs": 0, "completedJobs": 0, "conversations": 0, "unread_notifications": 0},
        "capabilities": {
            "canCreateJobs": True,
            "canAssignJobs": True,
            "canQuoteJobs": True,
